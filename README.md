...
Periodically checks for processes that are in D state (uninterruptable sleep, typically "waiting on IO" and then usually "waiting on disk"), straces them if they keep doing that, stops stracing and gives a summary when they stop doing so OR you press Ctrl-C (also helps summaries on processes that rarely or never exit, like databases and other daemons). 
